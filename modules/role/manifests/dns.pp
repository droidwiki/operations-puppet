...
class role::dns(
  ...
  bind::server::conf { '/etc/bind/named.conf':
    ...
    zones             => {
      ...
      'armakoth.wiki'  => [
        "type ${type}",
        'file "armakoth.wiki"',
        $type ? { 'master' => 'update-policy { grant letsencrypt. name _acme-challenge.armakoth.wiki. txt; }', default => '// omitting update-policy on slave' },
        $type ? { 'slave' => "masters { ${master_ip}; }", default => "allow-transfer { ${slave_ip}; }" }
      ],
    },
  }
  ...
  if $type == 'master' {
    ...
    exec { 'rndc thaw':
      ...
      user    => root,
      group   => root,
      require => File['/var/lib/bind/zones/go2tech.de', '/var/lib/bind/zones/droidwiki.org', '/var/lib/bind/zones/droid.wiki', '/var/lib/bind/zones/droid-wiki.org', '/var/lib/bind/zones/floriansw.de', '/var/lib/bind/zones/armakoth.wiki'],
    }

    bind::server::file { [ 'go2tech.de', 'droidwiki.org', 'droid.wiki', 'droid-wiki.org', 'floriansw.de', 'armakoth.wiki' ]:
